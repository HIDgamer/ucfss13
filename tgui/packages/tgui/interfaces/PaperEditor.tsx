...
export const PaperEditor = () => {
  ...
  const { mode, content, logo_wy, logo_wy_inv, logo_uscm, logo_upp, logo_cmb } =
    data;
  const editorRef = useRef<RichTextEditorHandle>(null);

  // A modest default width - the source assets render huge at their natural size, and the
  // sanitizer's width/height allow-list (10-2000px) lets players resize from here anyway.
  const logoImg = (src?: string) => `<img src="${src}" width="120">`;
  const logos: Logo[] = [
    { label: 'Weyland-Yutani', html: logoImg(logo_wy) },
    { label: 'Weyland-Yutani (inverted)', html: logoImg(logo_wy_inv) },
    { label: 'USCM', html: logoImg(logo_uscm) },
    { label: 'UPP', html: logoImg(logo_upp) },
    { label: 'CMB', html: logoImg(logo_cmb) },
  ];
        ...
        <Section
          ...
          buttons={
            <Button
              icon="check"
              onClick={() => {
                // Full replace, not append+clear - the editor already holds the complete,
                // already-edited document (see initialHtml below), so committing just saves
                // exactly what's currently shown, no reset needed.
