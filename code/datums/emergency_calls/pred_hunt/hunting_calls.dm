...
/datum/emergency_call/pred/mixed/create_member(datum/mind/player, turf/override_spawn_loc)
	...
	if(mercs < max_mercs && hunted.client.prefs.toggles_ert_pred & PLAY_MERC)
		mercs++
	...
	else if(upp < max_upp && hunted.client.prefs.toggles_ert_pred & PLAY_UPP)
		upp++
	...
	else if(royal_marines < max_royal_marines && hunted.client.prefs.toggles_ert_pred & PLAY_TWE)
		royal_marines++
	...
	else if(clf < max_clf && hunted.client.prefs.toggles_ert_pred & PLAY_CLF)
		clf++
