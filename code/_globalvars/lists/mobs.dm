...
/// Live-adjustable verbose tracing switch for the xeno AI movement/broadcast systems (reversal detection in ai_step(), queen_alert_turf changes in xeno_spawner.dm, fort-line build lifecycle) - separate from the server's blanket CONFIG_GET(flag/log_debug) so this can be toggled on/off for live oscillation-hunting without drowning in every other subsystem's debug output. Set from the admin AI Difficulty panel. Defaults off - this is meant to be turned on only while actively chasing a reported issue.
GLOBAL_VAR_INIT(ai_debug_pathing, FALSE)
