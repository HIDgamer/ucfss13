...
/obj/item/paper/ui_data(mob/user)
	...
	// The editor loads this in as its starting content - full in-place editing, not append-only,
	// so opening a paper that already has writing on it shows that writing instead of a blank box.
	data["content"] = info
...
/obj/item/paper/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	...
	switch(action)
		if("commit")
			...
			// Full in-place replace, not append - the editor loaded the previous info as its
			// starting content (see ui_data()), so what comes back here is the complete, already-
			// edited document, not just a new fragment to tack onto the end. Existing <font
			// color=...> spans from earlier writers keep their own color regardless of this outer
			// wrap, since an inner font tag's color always wins over an outer one.
			info = html
			updateinfolinks()
			fields = 0
			calculate_fields()
