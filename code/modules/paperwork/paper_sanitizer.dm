...
/proc/sanitize_paper_html(html, iscrayon = FALSE)
	...
	var/static/regex/font_size_re = regex(@{"\ssize\s*=\s*["']?([1-7])["']?"}, "i")
	var/static/regex/img_re = regex(@{"^<img\s+src\s*=\s*["']?([^"'>\s]*)["']?"}, "i")
	var/static/regex/width_re = regex(@{"\swidth\s*=\s*["']?(\d+)(?:px)?["']?"}, "i")
	var/static/regex/height_re = regex(@{"\sheight\s*=\s*["']?(\d+)(?:px)?["']?"}, "i")
	...
	while(tag_re.Find(html, pos))
		...
		if(tagname_re.Find(match_text))
			...
			if(full_name in allowed_plain)
			...
			else if(!iscrayon && !is_close && tagname == "font")
				if(font_re.Find(match_text))
				...
				else if(font_size_re.Find(match_text))
					replacement = "<font size=\"[font_size_re.group[1]]\">"
			else if(!iscrayon && !is_close && tagname == "img" && img_re.Find(match_text))
				var/src_url = img_re.group[1]
				if(src_url in allowed_logo_urls)
					var/size_attrs = ""
					if(width_re.Find(match_text))
						var/w = text2num(width_re.group[1])
						if(w >= 10 && w <= 2000)
							size_attrs += " width=\"[w]\""
					if(height_re.Find(match_text))
						var/h = text2num(height_re.group[1])
						if(h >= 10 && h <= 2000)
							size_attrs += " height=\"[h]\""
					replacement = "<img src=\"[src_url]\"[size_attrs]>"
