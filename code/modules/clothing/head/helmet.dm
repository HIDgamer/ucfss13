...
/obj/item/clothing/head/helmet/marine
	...
	/// The dmi where the helmet band overlay is contained
...
/obj/item/clothing/head/helmet/marine/proc/add_hugger_damage() //This is called in XenoFacehuggers.dm to first add the overlay and set the var.
	if(flags_marine_helmet & HELMET_DAMAGE_OVERLAY && !(flags_marine_helmet & HELMET_IS_DAMAGED))
		helmet_overlays["damage"] = image('icons/obj/items/clothing/hats/hats_by_map/classic.dmi',icon_state = "hugger_damage")
...
/obj/item/clothing/head/helmet/marine/leader/CO
	...
	flags_atom = NO_SNOW_TYPE|NO_NAME_OVERRIDE
	built_in_visors = list(new /obj/item/device/helmet_visor, new /obj/item/device/helmet_visor/medical/advanced, new /obj/item/device/helmet_visor/security, new /obj/item/device/helmet_visor/welding_visor)
...
/obj/item/clothing/head/helmet/marine/cbrn_hood
	...
	throwforce = 0
	// BLOCKGASEFFECT was missing despite the mask's own flavor text claiming gas filtration -
	// without it this piece didn't actually filter anything (breathe()'s own ambient-smoke check,
	// handle_breath.dm, and is_gas_infection_protected(), zombie_burster.dm, both key off this).
