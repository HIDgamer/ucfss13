...
/datum/action/innate/message_squad/action_activate()
	...
	if(human_owner.assigned_squad)
		...
		if(squads_being_overwatched_by_me.len >= 2)
			squads_being_overwatched_by_me.Add("All Squads")
	...
	var/text = tgui_input_text(human_owner, "Maximum message length [MAX_COMMAND_MESSAGE_LEN]", "Send message to [choice ? choice : (squads_being_overwatched_by_me.len ? squads_being_overwatched_by_me[1] : "team")]", max_length = MAX_COMMAND_MESSAGE_LEN, multiline = TRUE)
	if(!text)
		return
	if(!can_use_action())
		return
...
#undef MAX_COMMAND_MESSAGE_LEN
#undef COOLDOWN_HUD_LENGTH
