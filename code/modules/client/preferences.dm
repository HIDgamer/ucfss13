...
/datum/preferences
	...
	var/predator_mask_accessory_type = 0
	...
	//Queen name preferences
	var/show_queen_name = FALSE
	//ERT Predator toggles
	var/toggles_ert_pred = 0
	//Night vision preferences
	var/nightvision_preference = 0
	var/list/nvcolor_list = list()
	var/night_vision_color = null
	var/list/nv_color_list = list()
	//Fax responder settings
	var/fax_name_uscm = ""
	var/fax_name_pvst = ""
	var/fax_name_wy = ""
	var/fax_name_upp = ""
...
/datum/preferences/proc/ShowChoices(mob/user)
	...
	switch(current_menu)
		...
		if(MENU_SETTINGS)
			...
			dat += "<b>Ghost Announcement Clarity:</b> <a href='?_src_=prefs;preference=ghost_announceclarity'><b>[(toggles_chat & CHAT_GHOSTANNOUNCECLARITY) ? "Full Clarity" : "Potentially Garbled"] announcements</b></a><br>"
		...
		if("input")
			...
			switch(href_list["preference"])
				...
				if("limbs")
					...
					var/new_state = tgui_input_list(user, "What state do you wish the limb to be in?", list("Normal","Prothesis"))
					if(!new_state) return
