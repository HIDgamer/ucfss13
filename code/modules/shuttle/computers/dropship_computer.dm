	...
	for(var/obj/docking_port/stationary/dock in compatible_landing_zones)
		...
		for(var/obj/docking_port/mobile/other_shuttle in SSshuttle.mobile)
			if(dock == other_shuttle.destination)
				dock_reserved = TRUE
				break
		// An outer hangar pad can look empty on its own while the bay behind it is still occupied
		// (e.g. a resident shuttle sitting at its inner dock, never having lowered out) - a ship
		// could then be sent there and fly the whole way over with nowhere to actually land.
		var/bay_occupied = FALSE
		if(istype(dock, /obj/docking_port/stationary/marine_dropship/airlock/outer))
			var/obj/docking_port/stationary/marine_dropship/airlock/outer/outer_dock = dock
			if(outer_dock.linked_inner?.get_docked())
				bay_occupied = TRUE
		var/can_dock = shuttle?.canDock(dock)
		var/list/dockinfo = list(
			"id" = dock.id,
			"name" = dock.name,
			"available" = can_dock == SHUTTLE_CAN_DOCK && !dock_reserved && !bay_occupied,
			"error" = can_dock,
		)
