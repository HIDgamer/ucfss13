...
/obj/docking_port/stationary/marine_dropship/airlock/inner/proc/ceremony_satisfied_for(obj/docking_port/mobile/marine_dropship/requesting_shuttle)
	...
	if(!current_dock)
		...
		// gating this the same way as an in-flight ship would just permanently break shuttle
		// (re)loading - see the incident this comment was added for.
		return TRUE
	...
	if(resync_physical)
		...
		for(var/frame_key in inner_airlock_turf_lists)
			if(frame_key == "shuttle")
				continue
			for(var/turf/open/floor/hangar_airlock/inner/hangar_turf as anything in inner_airlock_turf_lists[frame_key])
				hangar_turf.open = FALSE
				hangar_turf.can_bloody = TRUE
		for(var/turf/open/floor/hangar_airlock/outer/hangar_turf as anything in outer_airlock_turf_lists)
			hangar_turf.open = FALSE
			hangar_turf.can_bloody = TRUE
...
/obj/docking_port/stationary/marine_dropship/airlock/inner/Initialize(mapload)
	...
	// handled live by canDock()'s spatial check, not by list membership. Unregistering a
	// non-roundstart airlock used to leave it with no path back to its own first registration.
