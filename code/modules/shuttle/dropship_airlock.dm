...
/obj/docking_port/stationary/marine_dropship/airlock/inner
	...
	/// Overall cycle status - see AIRLOCK_STATE_* in code/__DEFINES/dropships.dm. Set by resync_airlock_apparatus().
	var/airlock_state = AIRLOCK_STATE_IDLE
	/// TRUE only while this airlock's own delayed_height_decrease()/delayed_height_increase()/
	/// delayed_disengage_clamps() is the one calling initiate_docking() - lets on_arrival()/
	/// on_departure() tell an expected internal hop apart from an external one (admin force-move,
	/// hijack) that bypassed the cooperative update_*() machinery entirely.
	var/internal_relocation_in_progress = FALSE
	...
	var/list/tracked_static_alpha_masks = null
	var/list/tracked_airlock_projectors = null
	...
	/// Rate-limits sequence_violation_shake() so repeatedly mashing a button that's actually
	/// blocked (these checks return before processing is ever set) can't chain-trigger the
	/// ship-wide shake every single click.
	COOLDOWN_DECLARE(sequence_violation_shake_cooldown)
...
// Physical consequence for a genuine sequence violation (as opposed to just being busy, or
// something else legitimately occupying the airlock right now) - same effect as the admin
// "shake ship" verb, just triggered automatically instead of by an admin.
/obj/docking_port/stationary/marine_dropship/airlock/inner/proc/sequence_violation_shake()
	if(!COOLDOWN_FINISHED(src, sequence_violation_shake_cooldown))
		return
	COOLDOWN_START(src, sequence_violation_shake_cooldown, 10 SECONDS)
	// Scoped to whoever's actually on the dropship right now, not the whole crew - a misclick on
	// this one console shouldn't knock down every marine on the ship. Same turf-through-area walk
	// turbulence_sort_affected() (dropship.dm) already uses to find who's aboard.
	var/obj/docking_port/mobile/marine_dropship/ship = get_docked() || linked_outer.get_docked()
	if(!ship)
		return
	var/list/mobs_on_ship = list()
	for(var/area/shuttle_area as anything in ship.shuttle_areas)
		for(var/turf/shuttle_turf in shuttle_area)
			for(var/mob/living/carbon/mob_on_ship in shuttle_turf)
				mobs_on_ship += mob_on_ship
	if(!length(mobs_on_ship))
		return
	shakeship(3, 5, TRUE, restrict_to_mobs = mobs_on_ship)
...
/obj/docking_port/stationary/marine_dropship/airlock/inner/proc/delayed_height_decrease()
	if(COOLDOWN_FINISHED(src, dropship_airlock_cooldown))
		internal_relocation_in_progress = TRUE
...
/obj/docking_port/stationary/marine_dropship/airlock/inner/proc/delayed_disengage_clamps()
	...
	docked_mobile = null
	lowered_dropship = FALSE
	disengaged_clamps = FALSE
	if(open_outer_airlock)
		linked_outer.get_outer_airlock_turf_lists()
		update_outer_airlock(FALSE, forced = TRUE)
		return // the closing transition's own completion will call end_of_interaction()
...
/obj/docking_port/stationary/marine_dropship/airlock/inner/proc/get_inner_airlock_turf_lists()
	for(var/obj/effect/hangar_airlock/height_mask/static_alpha/stale_mask as anything in tracked_static_alpha_masks)
		qdel(stale_mask)
	tracked_static_alpha_masks = list()
	...
	for(var/turf/turf as anything in block(DROPSHIP_AIRLOCK_BOUNDS))
		if(istype(turf.loc, /area/shuttle))
			...
			tracked_static_alpha_masks += new /obj/effect/hangar_airlock/height_mask/static_alpha(turf)
...
/**
 * Whether requesting_shuttle is allowed to dock at THIS inner port right now - live, side-effect-
 * free, computed fresh every call (no caching/registration trick). A ship may only reach the inner
 * dock by having physically come up through this exact airlock's own outer dock (i.e. it's
 * currently sitting on linked_outer, about to raise) - anything else (a fresh flight from an LZ, a
 * different hangar's outer, transit) must dock at the outer port and go through the raise sequence
 * first. See canDock() on /obj/docking_port/mobile/marine_dropship for the one chokepoint this gates.
 */
/obj/docking_port/stationary/marine_dropship/airlock/inner/proc/ceremony_satisfied_for(obj/docking_port/mobile/marine_dropship/requesting_shuttle)
	var/obj/docking_port/stationary/current_dock = requesting_shuttle.get_docked()
	if(!current_dock)
		// A shuttle with no current dock at all has never been anywhere - it's being freshly
		// instantiated (roundstart template load, or an admin replacing a destroyed dropship via
		// the Shuttle Manipulator, both of which route through this same canDock() check via
		// SSshuttle.action_load()). There's no outer-then-raise cycle for it to have skipped, so
		// gating this the same way as an in-flight ship would just permanently break shuttle (re)loading.
		return TRUE
	return current_dock == linked_outer

/**
 * One idempotent recovery point for the whole airlock cycle. Re-derives the 5 phase booleans from
 * physical evidence (is a ship docked at inner? at outer? neither?) so they can never disagree with
 * where the ship actually is, and always broadcasts a destination-list refresh since a resync always
 * means availability may have changed.
 *
 * `resync_physical` additionally snaps the turfs/effects/floodlights back to a fully-closed resting
 * state and clears any in-flight timer/processing/autopilot-stage tracking - safe for a genuinely
 * interrupted cycle (admin force-move, hijack, force_process(), stuck "processing" recovery), but
 * NOT safe to run mid-way through a normal, uninterrupted sequence, since it would fast-forward past
 * whatever step hasn't happened yet (e.g. forcing "inner closed" before the sequence's own "close
 * inner" step has actually run). Callers on the normal internal-relocation path (on_arrival() for an
 * expected hop) should omit it and keep relying on their own existing step-specific logic; only pass
 * it from a path that's establishing state after something bypassed the cooperative machinery.
 */
/obj/docking_port/stationary/marine_dropship/airlock/inner/proc/resync_airlock_apparatus(reason, resync_physical = FALSE)
	var/obj/docking_port/mobile/marine_dropship/ship_at_inner = get_docked()
	var/obj/docking_port/mobile/marine_dropship/ship_at_outer = linked_outer.get_docked()
	var/target_lowered = (ship_at_outer && !ship_at_inner)

	docked_mobile = target_lowered ? ship_at_outer : null
	playing_airlock_alarm = FALSE
	open_inner_airlock = FALSE
	lowered_dropship = target_lowered
	open_outer_airlock = FALSE
	disengaged_clamps = FALSE
	...
	// Kept unconditional (unlike the rest of the physical resync below) - the logical state above is
	// always set unconditionally too, so a caller that skips resync_physical (e.g. an internal raise
	// hop) would otherwise leave the sprite showing the airlock's previous state indefinitely, even
	// though it's already logically closed. The turf state has to be synced unconditionally right
	// alongside the sprite, not just the icon - otherwise the door reads as closed but is still
	// passable/fall-through underneath, which is worse than the original desync (it actively lies).
	// Placed after the resync_physical rebuild above so a freshly-rebuilt list (population happens
	// only there) still gets its turfs marked closed in the same call, not left at their scan default.
	if(inner_airlock_effect)
		inner_airlock_effect.icon_state = "close"
	if(outer_airlock_effect)
		outer_airlock_effect.icon_state = "close"

	// No "as anything" on either loop below - these lists aren't guaranteed single-typed at the
	// point an automatic (resync_physical=FALSE) resync reaches them: their own rebuild is still
	// gated behind resync_physical above, so this can run against a stale, not-just-rebuilt list
	// that (e.g. via get_outer_airlock_turf_lists()'s own dual istype-outer-OR-area-shuttle
	// condition) may still hold a /turf/closed/shuttle/... entry. "as anything" would skip DM's
	// normal implicit type-filter and crash on hangar_turf.open for a turf type that doesn't declare that var.
	if(inner_airlock_turf_lists)
		for(var/frame_key in inner_airlock_turf_lists)
			if(frame_key == "shuttle")
				continue
			for(var/turf/open/floor/hangar_airlock/inner/hangar_turf in inner_airlock_turf_lists[frame_key])
				hangar_turf.open = FALSE
				hangar_turf.can_bloody = TRUE
	if(outer_airlock_turf_lists)
		for(var/turf/open/floor/hangar_airlock/outer/hangar_turf in outer_airlock_turf_lists)
			hangar_turf.open = FALSE
			hangar_turf.can_bloody = TRUE

	// playing_airlock_alarm = FALSE above is unconditional, so its physical side effects (normally
	// driven by delayed_airlock_alarm(), which resync never calls) have to follow just as
	// unconditionally - otherwise an automatic resync (e.g. the inner airlock auto-closing on a
	// raise/lower hop) sets the flag off while leaving the caution lights spinning, the railings
	// extended, and the alarm sound looping indefinitely, with no way to stop them short of
	// manually toggling the alarm button on and back off.
	var/obj/structure/machinery/floodlight/landing/dropship_airlock/floodlight
	for(floodlight as anything in floodlights)
		if(istype(floodlight.light?.our_mask, /atom/movable/lighting_mask/rotating_toggleable))
			var/atom/movable/lighting_mask/rotating_toggleable/rotating_mask = floodlight.light.our_mask
			if(rotating_mask.toggled)
				floodlight.toggle_rotating()
	...
	if(resync_physical)
		SSfz_transitions.toggle_selective_update(FALSE, dropship_airlock_id)

		automatic_process_stage_change = 0
		automatic_process_stage = 0

		if(reason == "admin_unlock")
			allow_processing_to_end = TRUE
			undergoing_admin_command = FALSE
		if(allow_processing_to_end)
			processing = FALSE

		airlock_state = (reason == "admin_lock") ? AIRLOCK_STATE_ADMIN_LOCKED : AIRLOCK_STATE_IDLE
...
/obj/docking_port/stationary/marine_dropship/airlock/inner/proc/force_process(command = FALSE) //a more generally-callable proc than autopilot, not alarmed, returns time till completion
	// Resync to physical ground truth before purging timers below - the purge can interrupt a
	// legitimately in-flight transition (e.g. the routine 120s auto-return grace timer firing
	// mid-animation), and without this the queued steps below would build on top of a turf/effect
	// state that no longer matches what the (now-cancelled) animation had actually reached.
	resync_airlock_apparatus("force_process", resync_physical = TRUE)
...
/obj/docking_port/stationary/marine_dropship/airlock/outer/proc/get_outer_airlock_turf_lists()
	for(var/obj/effect/projector/airlock/stale_projector as anything in linked_inner.tracked_airlock_projectors)
		qdel(stale_projector)
	linked_inner.tracked_airlock_projectors = list()
	...
	for(var/turf/turf as anything in block(DROPSHIP_AIRLOCK_BOUNDS))
		...
		linked_inner.tracked_airlock_projectors += new_projector

/*#############################################################################
Init/Arrival Backend Procs
#############################################################################*/
...
/obj/docking_port/stationary/marine_dropship/airlock/inner/Initialize(mapload)
	...
	// registered is left alone here (base Initialize() already registered it) - occupancy is
	// handled live by canDock()'s spatial check, not by list membership.

/obj/docking_port/stationary/marine_dropship/airlock/inner/on_departure(obj/docking_port/mobile/departing_shuttle)
	. = ..()
	// Every expected departure from this port goes through delayed_height_decrease(), which brackets
	// its own initiate_docking() call with internal_relocation_in_progress - anything else reaching
	// here (admin "Fly Shuttle", hijack, or any other direct initiate_docking()/request() call)
	// bypassed the cooperative update_*() machinery entirely and needs a full resync.
	if(!internal_relocation_in_progress)
		resync_airlock_apparatus("departure", resync_physical = TRUE)
...
/obj/docking_port/stationary/marine_dropship/airlock/inner/on_arrival(obj/docking_port/mobile/arriving_shuttle)
	...
	// A dropship arriving fully home means the airlock is at rest - resync unconditionally so stale
	// flags from an interrupted sequence can't deadlock every command against each other. Only force
	// the physical apparatus too (resync_physical) when this arrival did NOT come from our own
	// raise step - an expected internal hop already has the door correctly still open at this exact
	// point (it gets closed by a later, separate step), and forcing it shut here would skip that step.
	resync_airlock_apparatus("arrival", resync_physical = !internal_relocation_in_progress)
	// Clear stale height masks left over from a previous visit before laying down new ones for this arrival
	for(var/obj/effect/hangar_airlock/height_mask/dropship/stale_height_mask as anything in dropship_height_masks)
		qdel(stale_height_mask)
	dropship_height_masks = list()
...
/obj/docking_port/stationary/marine_dropship/airlock/outer/on_departure(obj/docking_port/mobile/departing_shuttle)
	. = ..()
	// Same rationale as the inner port's on_departure() - expected departures from here go through
	// delayed_height_increase() (raise) or delayed_disengage_clamps() (launch), both of which set
	// linked_inner.internal_relocation_in_progress around their own initiate_docking() call.
	if(!linked_inner.internal_relocation_in_progress)
		linked_inner.resync_airlock_apparatus("departure", resync_physical = TRUE)

/obj/docking_port/stationary/marine_dropship/airlock/outer/on_arrival(obj/docking_port/mobile/arriving_shuttle)
	. = ..()
	// Same rationale as the inner port's on_arrival() - resync unconditionally, but only force the
	// physical apparatus if this didn't come from our own lower step.
	linked_inner.resync_airlock_apparatus("arrival", resync_physical = !linked_inner.internal_relocation_in_progress)
	SSfz_transitions.fire()
	handle_obscuring_shuttle_turfs()
	// ..() above already queued an update_ambience() via the base marine_dropship on_arrival()
