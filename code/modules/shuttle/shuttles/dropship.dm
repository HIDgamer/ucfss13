/obj/docking_port/mobile/marine_dropship
	...
	/// TRUE only while admin_fly_shuttle() (an admin/event debug tool) is choosing/executing a
	/// destination for this ship - lets canDock() skip the hangar airlock's outer-then-raise
	/// ceremony requirement for admin-forced moves without weakening it for normal player flights.
	var/admin_bypass_ceremony = FALSE
...
/**
 * Every path that can move a dropship (a player's flight console, admin "Fly Shuttle") funnels
 * through here, so this is the one place that can require "must have come up through the hangar
 * airlock's outer dock and raised" for a return trip without needing separate checks in each
 * caller. See /obj/docking_port/stationary/marine_dropship/airlock/inner/proc/ceremony_satisfied_for().
 */
/obj/docking_port/mobile/marine_dropship/canDock(obj/docking_port/stationary/S)
	. = ..()
	if(. != SHUTTLE_CAN_DOCK)
		return
	if(admin_bypass_ceremony)
		return
	if(istype(S, /obj/docking_port/stationary/marine_dropship/airlock/inner))
		var/obj/docking_port/stationary/marine_dropship/airlock/inner/inner_port = S
		if(!inner_port.ceremony_satisfied_for(src))
			return SHUTTLE_AIRLOCK_CEREMONY_REQUIRED

/// Admin/event debug tool - allowed to bypass the hangar airlock ceremony outright, since it's
/// meant to override normal flight rules for testing/event/stuck-round recovery, and on_departure()/
/// on_arrival()'s resync (see dropship_airlock.dm) already guarantees the airlock ends up in a
/// physically consistent state regardless of how the ship got there.
/obj/docking_port/mobile/marine_dropship/admin_fly_shuttle(mob/user)
	admin_bypass_ceremony = TRUE
	. = ..()
	admin_bypass_ceremony = FALSE
...
/obj/docking_port/stationary/marine_dropship/on_arrival(obj/docking_port/mobile/arriving_shuttle)
	...
	// Admin-forced movement (fast-travel, "Fly Shuttle") drives destination/mode directly and never
	// goes through the flight console's own ui_act("move")/delayed_disengage_clamps() bookkeeping, so
	// without this, any open flight computer UI keeps showing a stale destination list from whatever
	// it last computed - refresh unconditionally on every arrival regardless of how it got here.
	// Broadcast to every console, not just this ship's own - a dock becoming occupied/vacated can
	// change what a *different* ship's console should be showing too.
	...
	// initiate_docking() only calls setDir() to the new dock's orientation *after* on_arrival()
	// returns, and update_ambience()'s bounds check depends on that being current - defer to
	// avoid computing the shuttle's bounding box with a stale (pre-arrival) direction.
	addtimer(CALLBACK(arriving_shuttle, TYPE_PROC_REF(/obj/docking_port/mobile, update_ambience)), 0)
...
/obj/docking_port/stationary/marine_dropship/on_departure(obj/docking_port/mobile/departing_shuttle)
	...
	// Same rationale as on_arrival() - keep every open flight computer UI in sync regardless of
	// whether this departure was player- or admin-initiated.
	SSshuttle.refresh_dropship_destination_lists()

	// Same ordering caveat as on_arrival() - setDir() to the new dock hasn't run yet at this point.
	addtimer(CALLBACK(departing_shuttle, TYPE_PROC_REF(/obj/docking_port/mobile, update_ambience)), 0)
