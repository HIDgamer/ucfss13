...
/**
 * This fork has no real second z-level over any live map (see the port plan's Part E notes) - so
 * "altitude" here is purely visual/mechanical, not a real z-change: while airborne the vehicle
 * moves to ROOF_PLANE (the same plane building rooftops render on, see
 * code/modules/tents/deployed_tents.dm for the existing precedent) at ABOVE_FLY_LAYER, so it draws
 * over rooftops instead of under them, and can_move() below skips the normal dense-turf collision
 * check so it can actually path over buildings instead of stopping at their walls.
 */
/obj/vehicle/multitile/blackfoot/proc/update_flight_visuals()
	if(state == STATE_VTOL || state == STATE_FLIGHT || state == STATE_TAKEOFF_LANDING)
		plane = ROOF_PLANE
...
/obj/vehicle/multitile/blackfoot/can_move(direction)
	if(state != STATE_FLIGHT && state != STATE_VTOL && state != STATE_TAKEOFF_LANDING)
		return ..()

	// Airborne: no real altitude change, so skip the normal dense-turf Enter() check entirely -
	// that's what actually lets it path over buildings instead of ramming into their walls.
	return get_step(src, direction) ? TRUE : FALSE
...
/obj/vehicle/multitile/blackfoot/proc/crash()
	...
	cell_explosion(impact_turf, 400, 50, EXPLOSION_FALLOFF_SHAPE_LINEAR, null, create_cause_data("blackfoot crash"))
	qdel(shadow_holder)
	entrances = null
...
/obj/vehicle/multitile/blackfoot/before_move(direction)
	...
	var/turf/shadow_turf = get_step(get_turf(src), direction)
...
/// Keeps the rear door's own sprite state in sync; this fork has no /turf/open_space subsystem to render a live ground view through it.
...
/obj/vehicle/multitile/blackfoot/proc/start_takeoff()
	...
	// No real z-change (see update_flight_visuals()'s doc comment) - only the current tile needs
	// checking, not a stack of z-levels above it.
	if(!fits_in_turf(get_turf(src)))
...
/obj/vehicle/multitile/blackfoot/proc/finish_takeoff()
	...
	shadow_holder = new(get_turf(src))
	shadow_holder.dir = dir
	shadow_holder.icon_state = "[get_sprite_state()]_shadow"
...
/obj/vehicle/multitile/blackfoot/proc/start_landing()
	...
	new /obj/downwash_effect(landing_turf)
	busy = TRUE
	change_state(STATE_TAKEOFF_LANDING)
	update_icon()
