	...
	switch(choice)
		if("Enable Player Control")
			if(selected_inner.allow_processing_to_end)
				return
			// Always recovers to a known-good state instead of just clearing the two flags - fixes
			// the case where "Disable Player Control" (or anything else) left the airlock stuck.
			selected_inner.resync_airlock_apparatus("admin_unlock", resync_physical = TRUE)
