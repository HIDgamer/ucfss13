...
/proc/play_sound_vox(sentence, list/players, list/vox, client/requesting_client, volume = 100)
	if(!islist(players))
	...
	if(requesting_client && length(bad_words))
		var/missed_words = jointext(bad_words, ", ")
		to_chat(requesting_client, SPAN_WARNING("Couldn't find the sound files for: [missed_words]"))
