...
// Explicit status for the airlock's overall cycle, tracked alongside (not yet replacing) its
// individual phase booleans - set by resync_airlock_apparatus() so every interruption path has
// one authoritative place recording "is this airlock mid-sequence / admin-locked / just resynced".
#define AIRLOCK_STATE_IDLE 0
#define AIRLOCK_STATE_IN_SEQUENCE 1
#define AIRLOCK_STATE_ADMIN_LOCKED 2
#define AIRLOCK_STATE_NEEDS_RESYNC 3
