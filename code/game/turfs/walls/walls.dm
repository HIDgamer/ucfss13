/turf/closed/wall
	...
	var/list/noblend_objects = list(/obj/structure/machinery/door/window, /obj/structure/machinery/door/poddoor/railing/dropship_airlock) //Objects to avoid blending with (such as children of listed blend objects.

	///Whether this turf is currently being manipulated to prevent doubling up
	var/busy = FALSE

	/// Humans currently leaning on this wall, mapped to the direction (from the wall) they lean from
...
/turf/closed/wall/MouseDrop_T(mob/current_mob, mob/user)
	...
	if(direction == NORTH)
		hiding_human.add_filter("cutout", 1, alpha_mask_filter(icon = icon('icons/effects/wall_lean.dmi', "cutout")))
