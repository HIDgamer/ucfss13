...
/atom/proc/create_clone(obj/effect/projector/related_projector) //NOTE: Use only for turfs, otherwise use create_clone_movable
	...
	if(related_projector.modify_turf)
		...
		// create_clone() can be re-triggered for a src that already has a live facsimile (the
		// fz_transitions.dm re-creation guard isn't the only path that can call this) - without this,
		// the old facsimile is orphaned (never qdel'd) while a new one stacks on top of it on the same
		// target_turf, forever, since nothing else ever reaps a turf-mastered clone.
		if(clone)
			GLOB.clones_t.Remove(src)
			qdel(clone, TRUE)
			clone = null
		...
		// src's own appearance doesn't always carry an underlay to strip (e.g. a closed wall turf has none), so guard the index instead of assuming one exists.
		if(length(facsimile_turf.underlays))
		...
		// Tracked on the projector (see projector.dm's spawned_clone doc comment) because src (the
		// master turf) can be silently replaced by a real turf-swap (ChangeTurf()) before this
		// facsimile is ever torn down - src.clone alone isn't a reliable enough handle to guarantee
		// cleanup, but related_projector survives that swap as an independent movable.
		related_projector.spawned_clone = facsimile_turf
