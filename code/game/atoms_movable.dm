...
// Declared on /atom rather than /atom/movable: only touches src.clone (base /atom var), and
// fz_transitions.dm's cleanup pass calls this on target_clone.mstr whether it's a movable or a turf.
/atom/proc/destroy_clone()
	GLOB.clones.Remove(src.clone)
	GLOB.clones_t.Remove(src) // no-op for a movable-mastered clone; correct teardown for a turf-mastered one
	qdel(src.clone, TRUE)
	src.clone = null
