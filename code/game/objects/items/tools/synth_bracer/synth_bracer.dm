/obj/item/clothing/gloves/synth
	...
	/// Internal camera console for WY cameras
	var/obj/structure/machinery/computer/cameras/internal/internal_camera_console
	var/list/internal_camera_network = list(CAMERA_NET_COLONY, CAMERA_NET_ALMAYER, CAMERA_NET_BRIG, CAMERA_NET_ARES, CAMERA_NET_ALAMO)
...
/obj/item/clothing/gloves/synth/verb/remove_gloves()
	...
	var/turf/drop_turf = get_turf(usr)
	if(!drop_turf)
		return
	underglove.forceMove(drop_turf)
	underglove = null
