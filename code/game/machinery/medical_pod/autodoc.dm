...
/obj/structure/machinery/autodoc_console/ui_status(mob/user, datum/ui_state/state)
	. = ..()
	// Unskilled users get a read-only view instead of being refused entry.
...
/obj/structure/machinery/autodoc_console/ui_data(mob/user)
	...
	// Reuses generate_autodoc_surgery_list() (also used by the bodyscanner) to highlight indicated-but-not-queued procedures.
					...
					for(var/obj/limb/L in occupant.limbs)
						if(L)
							if(occupant.incision_depths[L.name] != SURGERY_DEPTH_SURFACE)
								...
								needed++
					// Unlike the other cases, this always adds the "unneeded" filler entry regardless of `needed`.
