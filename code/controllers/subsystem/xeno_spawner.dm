...
/**
 * "The spawner spawns all different hives at once, placing the whole operation into chaos" -
 * this used to loop every entry in GLOB.hive_datum (13 hive numbers, mobs.dm's XENO_HIVE_*
 * list, ALL unconditionally instantiated at global-var init regardless of whether they're
 * actually in play a given round - global_lists.dm) and reinforce every single one every
 * fire. Now targets exactly one hive, GLOB.xeno_spawner_hive (admin-selectable, see
 * event_tab.dm's set_spawner_hive) - null means no hive selected, spawn nothing.
 */
/datum/controller/subsystem/xeno_spawner/fire(resumed = FALSE)
	...
	if(!hive)
		return
	spawner_maintain_population(hive)
 ...
 */
/proc/spawner_maintain_population(datum/hive_status/hive)
	...
	// stalls reinforcement, just slows it.
	var/max_per_fire = max(1, round(XENO_SPAWNER_MAX_PER_FIRE * GLOB.ai_difficulty_multiplier))
