...
/datum/controller/subsystem/fz_transitions/fire(resumed = FALSE)
	...
	for(var/atom/movable/clone/target_clone as anything in GLOB.clones)
		...
		else if(target_clone.proj && target_clone.mstr.loc != target_clone.proj.loc)
			target_clone.mstr.destroy_clone() //Master has walked off the projected turf, the clone is stale and would otherwise never be cleaned up
		else
