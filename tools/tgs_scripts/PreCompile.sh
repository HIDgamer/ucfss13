...
env PKG_CONFIG_ALLOW_CROSS=1 ~/.cargo/bin/cargo build --release --target=i686-unknown-linux-gnu --features redis_pubsub,iconforge
...
echo "Compiling tgui..."
cd "$1"
chmod +x tools/bootstrap/javascript.sh  # Workaround for https://github.com/tgstation/tgstation-server/issues/1167
